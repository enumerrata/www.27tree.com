...
class Qa_ndl_command {
    ...
    public static function newQuestion($uid, $title, $content, $type) {
        ...
        //return false;
        //if (!safe::defend_string($type))
        //return false;

        $mysql = Conn_mysql::getInstance();
        $sql = sprintf("INSERT INTO `qa_question_list` (`uid`, `title`, `datetime`, `last_modify`, `tags`) VALUES (%s, '%s', %s, %s, '%s');", $uid, $title, "NOW()", "NOW()", $type);
        #echo $sql;
