...
//interface ref
include_once $_SERVER['DOCUMENT_ROOT'] . '/user/data/iBlog.php';
...
class Blog implements BlogClass {
    ...
    private final function initXMLData() {
        ...
        $this->origin = $_re['origin'] == '1' ? TRUE : FALSE;
        $this->path = self::Get_blogFilePath($this->Submit_date,self::$blogId);
        ...
        if ($this->full) {
            if (!@$this->XML = simplexml_load_file($this->path)) {
                ...
                $this->parseVersion = (double) $_parseVersion;
            }
        }
    }

    public static function Get_blogFilePath($datetime, $blogid) {
        return $_SERVER['DOCUMENT_ROOT']
                . DIRECTORY_SEPARATOR . 'not'
                . DIRECTORY_SEPARATOR . 'blog_file'
                . DIRECTORY_SEPARATOR . $datetime
                . DIRECTORY_SEPARATOR . 'blog_' . $blogid . '.xml';
    }
