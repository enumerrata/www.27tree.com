...
function m_g(){
    dojo.query("#usr-head-l").onmouseover(function(){
...
function m_submit_blog(){
    ...
    // 按钮事件绑定
    $.getScript('/jss/jUploader.min.js',function(){
        var n_link = $('#rm-smblog-attach-link');

        $.jUploader({
            button: 'rm-smblog-attach-btn',
            action: '/user/api/upload/rm_smblog_attach_upload.php',
            onUpload : function(){
                n_link.show();
            },
            onComplete: function (fileName, response) {
                $('span',n_link).addClass('tags-bg');
                $('span',n_link).html(response.file_url);
                n_link.show();
                $('#rm-smblog-attach-btn').hide();
            }
        });
    });
