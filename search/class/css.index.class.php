...
class CloudSearchIndex {
    ...
    public function addDocument($docId, $fields) {
        $docs[] = $this->makeDocument('add', (string) $docId, $fields, NULL);
     ...
     */
    public function search($query, $page = NULL, $pageSize = NULL, $sort = NULL, $filter = NULL, $facet = NULL, $fetchFields = NULL) {
        $args = array();
        if (NULL != $page) {
            $this->_checkPageClause($page);
            $args['page'] = $page;
        ...
        if (NULL != $facet) {
            ...
            $mappedFacet = array_map(
                    create_function('$key, $value', 'return $key.":".$value;'), array_keys($facet), array_values($facet)
            );
